...
bool HVideoEncoder::SetupEncoder(IBaseFilter *filter)
{
	...
	captureInfo.callback           = fastdelegate::MakeDelegate( this, &HVideoEncoder::Receive );
...
void HVideoEncoder::Receive(IMediaSample *s)
{
    ...
    spinlock::simple_lock(packetMutex);
	packets.emplace_back(data, size);
    spinlock::simple_unlock(packetMutex);
}
		...
		EncoderPacket &packet, bool &new_packet)
{
    ...
    spinlock::simple_lock(packetMutex);
	if (packets.size() > 0) {
		curPacket = std::move(packets.front());
