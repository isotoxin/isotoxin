...
static bool EnumVideoCap(std::vector<VideoInfo> &caps,
		const AM_MEDIA_TYPE &mt, const BYTE *data)
{
	VideoInfo info;
...
#ifdef AUDIO_CAPTURE
static bool EnumAudioCap(std::vector<AudioInfo> &caps,
		const AM_MEDIA_TYPE &mt, const BYTE *data)
{
	AudioInfo info;
...
{
    spinlock::auto_lock_write lock(enumMutex);
