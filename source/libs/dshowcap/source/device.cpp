...
void HDevice::Receive(bool isVideo, IMediaSample *sample)
{
...
#else
    bool encoded = isVideo ?
        ((int)videoConfig.format >= 400) : false;

    if (!isVideo || !videoConfig.callback)
        return;
#endif
	...
	if (encoded) {
#ifdef AUDIO_CAPTURE
		DeviceEncodedData &data = isVideo ? encodedVideo : encodedAudio;
#else
        DeviceEncodedData &data = encodedVideo;
#endif
...
bool HDevice::SetupVideoCapture(IBaseFilter *filter, VideoConfig &config)
{
	...
	PinCaptureInfo info;
    info.callback = fastdelegate::MakeDelegate(this, &HDevice::ReceiveVideo);
...
bool HDevice::SetupAudioCapture(IBaseFilter *filter, AudioConfig &config)
{
	...
	PinCaptureInfo info;
	info.callback          = fastdelegate::MakeDelegate(this, &HDevice::ReceiveNonVideo);
