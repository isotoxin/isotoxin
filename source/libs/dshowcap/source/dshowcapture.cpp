...
static void OpenPropertyPages(HWND hwnd, IUnknown *propertyObject)
{
	...
	if (pages != NULL) {
		if (SUCCEEDED(pages->GetPages(&cauuid)) && cauuid.cElems) {
			OleCreatePropertyFrame(hwnd, 0, 0, L"", 1,
...
#ifdef AUDIO_CAPTURE
static bool EnumAudioDevice(std::vector<AudioDevice> &devices,
		IBaseFilter *filter,
		const wchar_t *deviceName,
		const wchar_t *devicePath)
{
...
bool Device::EnumAudioDevices(std::vector<AudioDevice> &devices)
{
	devices.clear();
