...
/*
  ...
  in russian:
  вобщем, тут конечно дикий изврат с шаблонами. попробую объ€снить
  MAKE_ROOT и MAKE_CHILD по сути ничем не отличаютс€, кроме того, какой движок пр€моугольнику они создают.
  template 1 и template 3 эквивалентны и используютс€ дл€ создани€ пр€моугольника без параметров, точнее с дефолтовым параметром в виде структуры initial_rect_data_s
  веселье начинаетс€, когда нужно передать дополнительные параметры в конструктор пр€моугольника. ¬от тут начинают работать template 2 и/или template 4
  по сути эти шаблоны - это предки дл€ специализированных структур с дополнительными параметрами
  как можно заметить при внимательном рассмотрении, эти шаблоны по сути специализаци€ MAKE_ROOT и MAKE_CHILD, но специальным враппером: newrectkitchen::rectwrapper
  такой изврат позвол€ет использовать вынести дефолтовую инициализацию движка в отдельную функцию init вылывать ее из специализированного потомка
