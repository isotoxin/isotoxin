		...
		for (; cur_text_index<text.l; ++cur_text_index)
		{
			...
			if (!search_rects && line_width > cur_max_line_len && last_line.count() > 1) // check special case: 1 symbol cannot be inserted to line
			{
                ...
                } else
                {
					...
					{
						//Ïåðåíîñ ñëîâ ïî ñëîãàì ðåàëèçîâàí íà îñíîâå óïðîù¸ííîãî àëãîðèòìà Ï.Õðèñòîâà http://sites.google.com/site/foliantapp/project-updates/hyphenation
						...
						for (; i<nn; i++)
						{
							...
							if (wcschr(L"àå¸èîóûýþÿaeiouyÀÅ¨ÈÎÓÛÝÞßAEIOUY", ch))
								charclass[i] = 'g';
							else if (wcschr(L"áâãäæçêëìíïðñòôõö÷øùbcdfghjklmnpqrstvwxzÁÂÃÄÆÇÊËÌÍÏÐÑÒÔÕÖ×ØÙBCDFGHJKLMNPQRSTVWXZ", ch))
								charclass[i] = 's';
							else if (wcschr(L"éüúÉÜÚ", ch))
								charclass[i] = 'x';
							else if (ch == '-')
								charclass[i] = '-';
						...
						for (int i=0; i<nn-2; i++)//[x, l+l]
							if (charclass[i] == 'x')
								charclass[i] = '-';//âñòàâëÿåì - ïåðåä íà÷àëîì ñëåä. ñëîãà, à íå ïîñëå, ÷òîáû ýòîò àëãîðèòì áûë ýêâèâàëåíòåí âñòàâêå '-' ìåæäó ñëîãàìè, èíà÷å â ïðîñòåéøèõ ñëó÷àÿõ íàïð. [êàòàëîã/sgsgsgs] áóäåò òîëüêî îäèí ïåðåíîñ âìåñòî äâóõ âîçìîæíûõ
