...
template <class NUM> INLINE NUM AngleNorm(NUM a) // Ќормировать угол. ¬озращает от -pi до +pi
{
...
template <class NUM> INLINE NUM AngleDist(NUM from,NUM to) // ƒистанци€ между углами. ¬озращает от -pi до +pi
{
...
template <> INLINE float tmin<float, float>(const float &x, const float &y)//используетс€ специализаци€ шаблона, т.к. просто перегруженна€ функци€ с float выбираетс€ там, где не надо, напр. дл€ min(int, short)
{
    float r;
