...
template<> struct MAKE_ROOT<dialog_smileselector_c> : public _PROOT(dialog_smileselector_c)
{
    ...
    RID editor;
    MAKE_ROOT(drawcollector &dcoll, const ts::irect &smilebuttonrect, RID editor) :_PROOT(dialog_smileselector_c)(dcoll), smilebuttonrect(smilebuttonrect), editor(editor) { init(false); }
    ~MAKE_ROOT() {}
};
