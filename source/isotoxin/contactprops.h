...
template<> struct MAKE_ROOT<dialog_contact_props_c> : public _PROOT(dialog_contact_props_c)
{
    ...
    MAKE_ROOT(drawchecker &dch) :_PROOT(dialog_contact_props_c)(dch) { init(false); }
    MAKE_ROOT(drawchecker &dch, const dialog_contactprops_params_s &prms) :_PROOT(dialog_contact_props_c)(dch), prms(prms) { init(false); }
    ~MAKE_ROOT() {}
};
